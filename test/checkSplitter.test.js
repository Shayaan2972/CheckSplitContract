...
contract("CheckSplitter", (accounts) => {
  ...
  it("should not allow non-owner to register participant", async () => {
    try {
      await checkSplitter.registerParticipant(participant2, {
        from: participant1,
      });
  ...
  it("should not allow duplicate registration of a participant", async () => {
    ...
    try {
      await checkSplitter.registerParticipant(participant1, { from: owner });
      assert.fail(
        "An error due to double registration should have been thrown."
      );
    } catch (error) {
  ...
  it("should not allow the bill to be initialized multiple times", async () => {
    ...
    try {
      await checkSplitter.initializeBill(oneEth, { from: owner });
      assert.fail(
        "An error due to instantiating more than once should have been thrown."
      );
    } catch (error) {
  ...
  it("should not allow bill to be initialized with an amount of 0", async () => {
    try {
      ...
      assert.fail(
        "An error due to instantiating with 0 eth should have been thrown."
      );
    } catch (error) {
      assert.include(
        error.message,
  ...
  it("should allow participant to withdraw", async () => {
    ...
    assert(
      ...
      "Balance should have increased after withdrawal"
    );
  });
  ...
  it("should allow multiple participants to contribute", async () => {
    ...
    assert.equal(
      details1.amountPaid.toString(),
      halfEth,
      "Participant 1 contribution incorrect"
    );
    assert.equal(
      details2.amountPaid.toString(),
      halfEth,
      "Participant 2 contribution incorrect"
    );
  });
  ...
  it("should allow a participant to contribute their exact share", async () => {
    ...
    assert.equal(
      details.amountPaid.toString(),
      sharePerParticipant,
      "Contribution does not match exact share"
    );
  });
  ...
  it("should correctly track remaining contract balance after contributions", async () => {
    ...
    assert.equal(
      ...
      oneEth,
      "Contract balance incorrect after contributions"
    );
  });
  ...
  it("should not allow non-participant to contribute", async () => {
    try {
      await checkSplitter.contribute(oneEth, {
        from: participant1,
        value: oneEth,
      });
      assert.fail("Should have thrown an error");
